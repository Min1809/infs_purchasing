...
class PurchaseOrderGroupInherit(models.Model):
    ...
    def get_rfqgp_url(self, company):
        base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
        return f"{base_url}/web#id={self.id}&model=purchase.order.group&view_type=form&cids={company.id}"

    def get_ip_rfqgp_url(self, company):
        web_ip_url = self.env['ir.config_parameter'].sudo().get_param('website.ip_address')
        return f"{web_ip_url}/web#id={self.id}&model=purchase.order.group&view_type=form&cids={company.id}"
    ...
    def _send_approval_notification(self):
        ...
        if send_approval_mails:
            ...
            if stage == 'submitted':
                self._send_approval_email('infs_purchasing.email_template_rfqgp_level_1_approval', 'infs_purchasing.purchase_supervisor_level_1')
            elif stage == 'approved_lvl_1':
                self._send_approval_email('infs_purchasing.email_template_rfqgp_level_2_approval', 'infs_purchasing.purchase_supervisor_level_2')
            elif stage == 'approved_lvl_2':
                self._send_approval_email('infs_purchasing.email_template_rfqgp_level_3_approval', 'infs_purchasing.purchase_supervisor_level_3')
            elif stage == 'approved':
                self.action_send_confirmed_mail_to_rfqgp()
